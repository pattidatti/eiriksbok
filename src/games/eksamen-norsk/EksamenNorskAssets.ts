...
import { laererDialogs, faglarerDialogs, sensorDialogs } from './EksamenNorskDialogs';
...
export function setupEksamenNorskScene(engine: GameEngineRef): void {
    // ═══════════════════════════════════════════════════════════════════════
    ...
    prepStation(engine, {
        id: 'b-tekster', model: 'chest', pos: [-4, 0, -1],
        prompt: 'Velg tekster fra biblioteket (E)', flag: FLAGS.HAR_TEKSTER, doneMonolog: 'm_tekster',
    });
    prepStation(engine, {
        id: 'b-paastand', model: 'lectern', pos: [4, 0, -1],
        prompt: 'Skriv en skarp påstand (E)', flag: FLAGS.HAR_PAASTAND, doneMonolog: 'm_paastand',
    });
    prepStation(engine, {
        id: 'b-tankekart', model: 'table', pos: [-4, 0, -5],
        prompt: 'Lag tankekart og koble til epoker (E)', flag: FLAGS.HAR_EPOKEKOBLING, doneMonolog: 'm_tankekart',
    });
    prepStation(engine, {
        id: 'b-baklomme', model: 'chest', pos: [4, 0, -5],
        prompt: 'Forbered baklomme-tekster (E)', flag: FLAGS.HAR_BAKLOMME, doneMonolog: 'm_baklomme',
    });
    ...
    addInteractable(engine, {
        ...
        model: 'table',
        pos: [0, 0, -6.5],
        prompt: 'Skriv stikkordlapp og øv (E)',
        onInteract: () => {
    ...
    registerMonolog(engine, 'm_oppgave', [
        'Oppgaven: "Drøft hvordan framgang og motgang framstilles i selvvalgte tekster fra ulike tidsperioder. Bruk fagbegreper."',
        'Temaet er framgang og motgang. Du tenker med en gang på tre tekster fra biblioteket: Askeladden, Karens jul og Sult.',
    ]);
    registerMonolog(engine, 'm_tekster', [
        'Du velger tre tekster fra biblioteket: Askeladden som kappåt med trollet (1843), Karens jul av Amalie Skram (1885) og Sult av Knut Hamsun (1890).',
        'Tre epoker, tre svar på det samme spørsmålet om framgang og motgang.',
    ]);
    registerMonolog(engine, 'm_paastand', [
        'Du spisser temaet til en skarp påstand: "Framgang flytter seg gjennom litteraturhistorien - fra en belønning du kan vinne med list og vilje, til noe samfunnet eller ditt eget hode kan nekte deg."',
    ]);
    registerMonolog(engine, 'm_tankekart', [
        'Du lager et tankekart for hver tekst: virkemiddel, budskap og epoke.',
        'Askeladden - kontrast, folkediktning. Karens jul - ironi, naturalisme. Sult - indre monolog, nyromantikk.',
    ]);
    registerMonolog(engine, 'm_baklomme', [
        'Du legger noen tekster i baklomma til samtalen: Aldri godt nok (2024) og Piken med svovelstikkene.',
        'Hvis faglærer ber om et eksempel til, er du klar.',
    ]);
    registerMonolog(engine, 'm_manus', [
        'Stikkordlappen sitter, og du har øvd på å snakke fritt. Døra til eksamensrommet er åpen når du er klar.',
    ]);
    registerMonolog(engine, 'm_les_oppgave_forst', [
        'Du bør lese oppgaven før du skriver stikkordlappen. Den ligger ved inngangen til forberedelsesrommet.',
    ]);
    registerMonolog(engine, 'm_laast_fag', [
        'Døra er låst. Du må trekke faget hos læreren først.',
    ]);
    registerMonolog(engine, 'm_laast_manus', [
        'Døra til eksamensrommet er låst. Du må iallfall ha skrevet en stikkordlapp før du går inn.',
    ]);
    ...
    wireChoice(dialogs, 'pres_apning', 0, 1, () => {
        if (engine.getFlag(FLAGS.HAR_PAASTAND)) {
    ...
    wireChoice(dialogs, 'pres_innhold', 0, 1, () => {
        if (engine.getFlag(FLAGS.HAR_EPOKEKOBLING)) {
            engine.setFlag(FLAGS.KOBLET_EPOKE, true);
            engine.setEmotion('faglarer', 'glad');
        }
    });
    ...
    wireChoice(dialogs, 'pres_kjerne', 0, 1, () => {
        engine.setFlag(FLAGS.KJERNE_RIKTIG, true);
        engine.setEmotion('sensor', 'surprised');
    });

    wireChoice(dialogs, 'pres_virkemiddel', 0, 0, () => {
        if (engine.getFlag(FLAGS.HAR_TEKSTER)) {
            engine.setFlag(FLAGS.BRUKTE_SITATER, true);
        }
    });
    ...
    wireChoice(dialogs, 'pres_avslutning', 0, 0, () => {
        if (engine.getFlag(FLAGS.HAR_PAASTAND)) {
            engine.setFlag(FLAGS.SIRKLET_TILBAKE, true);
            engine.setEmotion('faglarer', 'glad');
        }
    });

    // Presentasjonen er ferdig uansett hvilken avslutning eleven valgte.
    const onPresentasjonFerdig = (): void => {
    ...
    wireDialogEnd(dialogs, 'pres_avslutning_tilbake', onPresentasjonFerdig);
    wireDialogEnd(dialogs, 'pres_avslutning_takk', onPresentasjonFerdig);
...
function wireChoice(
    ...
    if (!choice) {
        console.warn(`[eksamen-norsk] wireChoice fant ikke ${key}[${variantIndex}].choices[${choiceIndex}]`);
        return;
    }
...
function wireDialogEnd(
    ...
    if (nodes.length === 0) {
        console.warn(`[eksamen-norsk] wireDialogEnd fant ikke dialog '${key}'`);
        return;
    }
