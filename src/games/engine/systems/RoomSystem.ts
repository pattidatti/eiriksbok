...
export interface BuiltRoom {
    ...
    floor: THREE.Mesh;
    wallBoxes: AABB2D[]; // kun veggsegment-bokser, ikke gulv/tak
...
export function buildRoom(
    ...
    for (const side of ['N', 'S', 'E', 'W'] as const) {
        ...
        for (const seg of segments) {
            ...
            // Auto-generer kollisjonsboks (world-koordinater). Player-radius 0.4 inkludert.
            const worldX = cx + localX;
            const worldZ = cz + localZ;
            const halfGeoW = geoW / 2 + 0.4;
            const halfGeoD = geoD / 2 + 0.4;
            const box: AABB2D = {
                minX: worldX - halfGeoW,
                maxX: worldX + halfGeoW,
                minZ: worldZ - halfGeoD,
                maxZ: worldZ + halfGeoD,
            };
            wallBoxes.push(box);
    ...
    return { def, group, roof, floor, wallBoxes, innerBounds };
}
